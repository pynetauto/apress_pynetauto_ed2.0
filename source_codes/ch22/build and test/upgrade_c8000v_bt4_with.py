...
def test_connectivity(device_list_netmiko):
    with open('reachable_ips_ssh.txt', 'w+') as f1, open('reachable_ips_telnet.txt', 'w+') as f2, open('unreachable_ips.txt', 'w+') as f3:
        for device in device_list_netmiko:
            ip = device['host'].strip()
            print(ip)
            resp = os.system('ping -c 4 ' + ip)
            if resp == 0:
                with open('reachable_ips_ssh.txt', 'a+') as f1:
                    for port in range(22, 23):
                        ...
                        try:
                            ...
                            with open('unreachable_ips.txt', 'a+') as f3:
                                f3.write(f"{ip} {port} closed\n")
                    with open('reachable_ips_telnet.txt', 'a+') as f2:
