...
# Function to check flash space on devices
...
# Function to upload IOS to devices
...
# Function to verify IOS MD5 checksum post-upload
...
# Function to capture post-reload show command outputs
...
# Function to change boot variable on devices
...
# Function to reload devices based on user input
...
read_info(uid, pwd, secret)
test_connectivity(device_list_netmiko)
newiossize = validate_md5(device_list)
check_flash(device_list_netmiko, newiossize)
upload_ios(device_list_netmiko, device_list)
verify_md5(device_list_netmiko, device_list)
reload_yes_no_in_parallel(device_list_netmiko, device_list)

tt_ios_upload = time.mktime(time.localtime()) - t2
print("Total Time : {0} seconds".format(tt_ios_upload))

tt = time.mktime(time.localtime()) - t1
print("Total time : {0} seconds".format(tt))