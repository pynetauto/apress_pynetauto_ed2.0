...
def send_sms(): 
    ...
    my_message = f"High CPU utilization notice, R1 has reached 90% CPU utilization."
    message = client.messages.create(body=my_message, from_=twilio_trial, to=my_smartphone)
    print(message.sid)

stream = os.popen('snmpwalk -v3 -l authPriv -u SNMPUser1 -a SHA -A "AUTHPass1"  -x AES -X "PRIVPass1" 192.168.183.10 1.3.6.1.4.1.9.9.109.1.1.1.1.5') 
output = stream.read() 
time.sleep(3) 
print("-"*80)
